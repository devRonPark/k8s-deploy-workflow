...
|---|---|---|---|
| **Platform Engineer** | 클러스터 표준(registry, namespace, ingress class, 리소스 정책) 준수 | Deployment Profile 작성·배포(3장), Profile을 팀 표준 템플릿으로 관리 | `11-deployment-profile.template.yaml`, `13-validation-report.yaml` |
| **DevOps Engineer** | CI/CD 파이프라인에 분석·렌더·검증을 편입, 배포 후 확인 자동화 | Repository-only 분석(2장) → Profile 병합(3장) → 배포 검증(4장) 전체 | CLI 종료 코드, `13-validation-report.yaml`, `16-repair-suggestions.yaml` |
| **Application Developer** | "내 앱이 K8s에서 뜨려면 뭐가 더 필요한가"를 파악 | Repository 분석 결과 검토(2장)로 component를 확인·수정하고 앱 수준 질문(포트, 헬스 endpoint)에 답변 | `10-unresolved-questions.yaml`, `06-component-model.yaml` |
...
② branch / commit SHA 선택   →  Step 0  repository snapshot 생성 (commit 고정)
                                ...
                                Step 6  사용자 확인용 분석 결과/질문 생성
                                        ...
                                        → 11-deployment-profile.template.yaml
...
⑥ Manifest 생성               →  12-generated-manifests/
⑦ Validation                  →  13-validation-report.yaml
⑧ 실패 시 Repair              →  16-repair-suggestions.yaml
```
...
9. **unresolved_questions.yaml 생성** (`10`) — 각 질문은 `id(Q-DB-001 등) / 대상 필드 / 질문 문안 / 이유 / 답변 형식 / 후보값 / blocking_level`을 갖는다. 함께 생성되는 `11-deployment-profile.template.yaml`에는 각 질문에 대응하는 빈 필드가 `# Q-xxx` 주석과 함께 배치되어 있어, 사용자는 질문을 읽고 템플릿을 채우기만 하면 된다.
...
사용자                              시스템
──────────────────────────────────  ────────────────────────────────────────
① 11-deployment-profile.template.yaml 을 복사해 작성 시작
...
1. **Profile 작성 시작** — 사용자는 빈 파일에서 시작하지 않는다. Reconciliation 이후 생성된 `11-deployment-profile.template.yaml`에는 이 Repository에 필요한 필드만, 대응 질문 ID 주석과 함께 배치되어 있다. Platform Engineer가 클러스터 표준 값(registry, ingress class, storage class, resource_policy)을, Developer가 앱 수준 값을 나눠 채우는 협업이 일반적이다.
...
$ preanalyzer analyze https://github.com/acme/shop-backend --ref v2.3.1
  ...
  # → reconciliation → render → validate 실행. LLM endpoint가 없으면 --no-llm으로 완주
  → repo-analysis-output/ 생성
  → "컴포넌트 2개 감지 (backend-api, frontend). Reconciliation conflict 1건.
     질문 7건 (required 5, optional 2).
     achieved_level: 1 — Ingress/Deployment 일부는 렌더 보류 (사유: registry, host 미공급)"

  사용자: 05-reconciliation-report.yaml, 06-component-model.yaml,
          08-dependency-model.yaml 검토
          — frontend가 잘못 dependency로 분류되었다면 user_decision으로 수정
          10-unresolved-questions.yaml 검토
          — Q-PORT-001의 후보(8080, low)가 맞는지 앱 코드로 확인해 답변 준비

─── 2일차: Platform Engineer ───────────────────────────────────────

  (generate-profile에 해당 — analyze가 이미 생성한 템플릿 사용)
  사용자: 11-deployment-profile.template.yaml 을 dev-profile.yaml로 복사,
...
$ preanalyzer merge-profile repo-analysis-output/ --profile dev-profile.yaml
  # Profile을 user_decision으로 병합 + Reconciliation + render + validate 재실행 (재분석 없음)
...
$ preanalyzer validate repo-analysis-output/12-generated-manifests --k8s-version 1.29
...
|---|---|---|---|
| `analyze` | snapshot→evidence→semantic→reconciliation→render→validate | `00`~`15` | 제공 (`--ref`, `--profile`, `--no-llm`) |
| `generate-profile` | Reconciliation의 일부 | `11-deployment-profile.template.yaml` | analyze에 포함 |
| `render` | 11 | `12-generated-manifests/` | analyze / merge-profile에 포함 |
| `validate` | 12 | `13-validation-report.yaml` | 독립 커맨드 제공 |
| `merge-profile` | Profile → Reconciliation → render → validate | 갱신된 `05`~`13` | 제공 |
| `deploy-check` | 13 | report 갱신 (Level 2) | checklist 기반 수동 → 2단계 자동화 |
| `smoke-test` | 14 | report 갱신 (Level 3) | plan 파일 기반 수동 → 2단계 자동화 |
| `repair` | 15 | `16-repair-suggestions.yaml` | 규칙 기반 + 오류 설명까지 → 2단계 확장 |
