...
한 문장 요약: **사용자는 Repository URL을 넣고, 시스템이 Evidence를 생성한 뒤 LLM Semantic Analysis와 Rule/LLM Reconciliation 결과를 먼저 검토한다. 사용자는 잘못 분석된 Component를 수정하고, Deployment Profile로 운영환경 값을 공급한 뒤, 템플릿 렌더링·검증·Repair를 거쳐 배포 가능성 수준(Level 0~3)을 확정한다.** 시스템은 어떤 단계에서도 사용자를 대신해 DB 접속 정보·registry·namespace·도메인·Secret 값을 추측하지 않는다(P5).
...
|---|---|---|---|
| **Platform Engineer** | 클러스터 표준(registry, namespace, ingress class, 리소스 정책) 준수 | Deployment Profile 작성·배포(3장), Profile을 팀 표준 템플릿으로 관리 | `profile/deployment-profile.yaml`, `validation/13-validation-report.yaml` |
| **DevOps Engineer** | CI/CD 파이프라인에 분석·렌더·검증을 편입, 배포 후 확인 자동화 | Repository-only 분석(2장) → Profile 병합(3장) → 배포 검증(4장) 전체 | CLI 종료 코드, `validation/13-validation-report.yaml`, repair suggestions |
| **Application Developer** | "내 앱이 K8s에서 뜨려면 뭐가 더 필요한가"를 파악 | Repository 분석 결과 검토(2장)로 component를 확인·수정하고 앱 수준 질문(포트, 헬스 endpoint)에 답변 | `agent/questions.yaml`, `06-component-model.yaml` |
| **Solution Architect** | 전환 대상 시스템의 컴포넌트 구조·의존성·아키텍처 결정 항목 파악 | Repository-only 분석(2장)의 reconciliation/intermediate model 검토, 아키텍처 결정 질문(DB 외부화, discovery 전환 등) 응답 | `05-reconciliation-report.yaml`, `08-dependency-model.yaml`, `09-kubernetes-intent.yaml` |
...
```text
사용자                          시스템 (k8s-agent prepare)
──────────────────────────────  ─────────────────────────────────────────────
① Repository URL 입력            
② branch / commit SHA 선택   →  Step 0  repository snapshot 생성 (commit 고정)
                                Step 1  artifact inventory 생성 (부재도 기록)
                                Step 2  Parser + Evidence 생성
                                        → 02-evidence-model.yaml
                                Step 3  Rule Inference
                                        → 03-rule-inference.yaml
                                Step 4  Evidence Bundle 기반 LLM Semantic Analysis
                                        → 04-semantic-analysis.yaml
                                Step 5  Rule + LLM Reconciliation
                                        → 05-reconciliation-report.yaml
                                        → 06/07/08/09 Intermediate Model
                                Step 6  사용자 확인용 분석 결과/질문 생성
                                        → agent/questions.yaml
                                        → profile/deployment-profile.yaml
③ Repository 분석 결과 검토   ←  component/dependency/reconciliation 결과
④ 잘못 분석된 Component 수정  →  user_decision으로 Reconciliation 재실행
⑤ Deployment Profile 작성     →  운영환경 값 공급(사용자/조직 표준)
⑥ Manifest 생성               →  generated/manifest-bundle.yaml
⑦ Validation                  →  validation/13-validation-report.yaml
⑧ 실패 시 Repair              →  repair suggestions
```
...
5. **Evidence 생성** (`02-evidence-model.yaml`) — 시스템이 Repository에서 관찰한 사실의 원장이다. 파일 존재/부재, Dockerfile EXPOSE, Compose depends_on, package dependency, env 참조가 `evidence_id`와 함께 기록된다. 이 단계에는 의미 판단이 없다.

6. **LLM Semantic Analysis** (`04-semantic-analysis.yaml`) — LLM은 Repository 전체가 아니라 Evidence Bundle을 보고 component boundary, role, runtime behavior, dependency semantics, deployment intent 후보를 해석한다. 모든 LLM 판단에는 `evidence_refs`가 있어야 하며, 근거 없는 판단은 폐기된다.

7. **Rule + LLM Reconciliation** (`05-reconciliation-report.yaml`) — Rule과 LLM이 같은 결론을 냈는지, 충돌했는지, 어떤 evidence로 채택/거부되었는지를 보여준다. 이 결과로 `06-component-model.yaml`, `07-runtime-model.yaml`, `08-dependency-model.yaml`, `09-kubernetes-intent.yaml`이 생성된다.

8. **분석 결과 사용자 확인** — 사용자는 먼저 component 목록, role, dependency graph, reconciliation conflict를 확인한다. 잘못 분석된 component boundary/role/dependency가 있으면 수정값을 입력하고, 그 수정은 `classification: user_decision`으로 기록되어 Reconciliation을 다시 통과한다.

9. **질문과 Profile 생성** — 각 질문은 `id(Q-DB-001 등) / 대상 필드 / 질문 문안 / 이유 / 답변 형식 / 후보값 / blocking_level`을 갖는다. Agent MVP에서는 run directory의 `agent/questions.yaml`과 `profile/deployment-profile.yaml`에 남으며, 사용자는 질문에 답하거나 Profile 값을 보강한 뒤 resume한다. 저수준 preanalyzer 단독 실행에서는 `10-unresolved-questions.yaml`과 `11-deployment-profile.yaml`을 확인한다.

10. **Level 1 확인** (`13-validation-report.yaml`) — 렌더 가능한 리소스(필수 값이 모두 확보된 것)는 렌더링되어 kubeconform·dry-run(client)을 통과하고 `achieved_level: 1`이 기록된다. 필수 값이 unresolved인 리소스(예: host 없는 Ingress)는 **렌더 보류**되고 사유가 기록된다. 사용자가 확인할 것: `achieved_level`과 `target_level`이 분리 기록되어 있고, 실행되지 않은 검증(`dry_run.server`, `deployment_check`, `smoke_test`)이 `skipped`/`not_run`으로 정직하게 표기되어 있는지.
...
| Repository 분석 결과(component/role/dependency)와 Reconciliation 근거 | 근거 없는 LLM 결론의 자동 채택 |
| 잘못 분석된 Component를 수정할 수 있는 user_decision 단계 | 운영환경 값의 자동 생성 |
| 무엇이 비어 있는지의 완전한 목록 (질문 + Profile) | DB host, registry, ingress host 값 (시스템은 절대 추측하지 않음) |
...
사용자                              시스템
──────────────────────────────────  ────────────────────────────────────────
① profile/deployment-profile.yaml 또는 answers file을 기준으로 작성 시작
② registry / namespace / ingress host / DB 접속 /
   Secret ref / storage class 입력
③ resume/prepare 재실행         →  Profile JSON Schema 검증 (위반 시 병합 전 거부)
                                    Step 10 병합: unresolved 필드 ← Profile 값(user_decision)
                                            resolved_by: deployment_profile 기록
                                            Reconciliation 재실행, 질문 재계산
                                    Step 8  Intermediate/Kubernetes Intent Model 재생성
                                    Step 11 manifest rendering (Ingress 등 보류분 포함)
                                    Step 12 validation
④ 갱신된 산출물 검토           ←  05~13 갱신 (00~04 불변)
⑤ Pod-runnable / Application-runnable 여부 확인 (4장으로 연결)
```

## 3.2 단계별 사용자 경험

1. **Profile 작성 시작** — 사용자는 빈 파일에서 시작하지 않는다. Agent MVP는 run directory에 `profile/deployment-profile.yaml`을 만들고, 질문은 `agent/questions.yaml`에 남긴다. Platform Engineer가 클러스터 표준 값(registry, ingress class, storage class, resource_policy)을, Developer가 앱 수준 값을 나눠 채우는 협업이 일반적이다.
...
4. **Reconciliation / Kubernetes Intent Model 재생성** — 채워진 필드는 `classification: user_decision`, `resolved_by: deployment_profile`로 기록되고, 해소된 질문은 목록에서 사라진다. 사용자가 확인할 것: 남은 질문 중 `blocking_level: application_runnable`이 0건인지 — 0건이어야 Level 2~3 검증 단계로 진입할 수 있다.
...
7. **Pod-runnable / Application-runnable 여부 확인** — 이 시점의 report는 `target_level: 2`(또는 3)를 기록하지만, `achieved_level`은 아직 1이다. **Level 2~3은 생성 시점의 약속이 아니라 배포 후 검증의 결과**이므로(설계 문서 8.5), 확정은 4장의 흐름에서 이루어진다. MVP에서는 이 배포 후 검증을 `14-deployment-readiness-checklist.md`의 절차에 따라 사용자가 수동 실행한다.

---

# 4. Post-deployment Validation 흐름

Profile 병합·렌더링·Level 1 검증을 통과한 manifest를 실제 클러스터에 적용해 Level 2~3을 확정하는 흐름이다(설계 문서 Step 13~15, 검증 체인 ⑥~⑧). MVP 범위에서는 시스템이 `14-deployment-readiness-checklist.md`와 `15-smoke-test-plan.yaml`을 **생성까지만** 하고, 실행은 사용자가 checklist를 따라 수동으로 수행한다. 2단계 로드맵에서 이 흐름 자체가 자동화된다(deploy-check / smoke-test 커맨드).
...
⑤ Service / Ingress 확인          endpoint 연결, 라우팅 도달성
        │
⑥ smoke test 실행                 15-smoke-test-plan.yaml의 검사
...
- **repair_suggestions.yaml** (`16-repair-suggestions.yaml`): 실패 시 생성. 알려진 오류 패턴은 규칙 기반 수리 항목으로(예: ImagePullBackOff → registry/credential/태그 점검), 규칙으로 못 잡는 오류만 LLM의 schema-constrained patch 제안으로 기록된다. patch는 항상 **Intent Model 또는 Profile에 대한 수정 제안**이며 YAML을 직접 고치지 않는다 — 사용자가 제안을 검토·채택하면 렌더링부터 재실행된다.
...
| **Pod CrashLoopBackOff** | 이벤트·로그·exit code 수집 → 규칙 기반 매핑(command/env 점검) 우선, 미해결 시 LLM patch 제안 | `16-repair-suggestions.yaml` 검토 → Intent/Profile patch 채택 → 재렌더·재배포 |
...
모든 산출물 세트의 공식 수준은 `13-validation-report.yaml`의 `achieved_level`이다. `target_level`(목표)과 분리 기록되므로, 사용자는 "지금 어디까지 왔고 무엇이 남았는가"를 한 필드로 확인한다.
...
현재 CLI 기준의 전체 여정. `k8s-agent prepare`는 source 획득부터 분석, 질문/Profile, 렌더링, 검증까지 가능한 곳을 이어서 실행한다. 단계별 제어가 필요하면 `analyze` → `plan` → `generate` → `validate`를 분리해서 실행한다. 실제 클러스터 배포 확인과 smoke test 자동화는 아직 MVP 밖이며, checklist·plan 파일 기반 수동 절차로 대응한다.

```text
─── 1일차: Application Developer ───────────────────────────────────

$ k8s-agent prepare --repo-url https://github.com/acme/shop-backend --ref v2.3.1 --target development
  # snapshot → inventory → evidence → rule inference → semantic analysis
  # → reconciliation → render → validate 실행. LLM endpoint가 없으면 결정론 경로로 완주
  → run_id와 run_root 출력
  → "state=WAITING_FOR_USER 또는 READY. 다음 행동은 next=...에 표시"

  사용자: run_root 아래 analysis/, agent/questions.yaml,
          profile/deployment-profile.yaml, validation/13-validation-report.yaml 검토
          — frontend가 잘못 dependency로 분류되었다면 user_decision으로 수정
          agent/questions.yaml 검토
          — Q-PORT-001의 후보(8080, low)가 맞는지 앱 코드로 확인해 답변 준비

─── 2일차: Platform Engineer ───────────────────────────────────────

  사용자: answers.yaml 또는 profile/deployment-profile.yaml에
          registry / namespace / ingress host / DB secret_ref / storage class 기입

$ k8s-agent prepare --local-path ./shop-backend --target development --non-interactive --answers-file answers.yaml
  # 자동 실행에서는 답변 파일을 입력해 질문 단계에서 멈추지 않게 한다.
  → "state=READY 또는 WAITING_FOR_USER. 남은 질문과 block reason은 agent/questions.yaml 참조"

$ k8s-agent status <run-id>
  # 현재 상태, 검증 결과, 리소스 수 확인

$ k8s-agent export <run-id> --output out/manifests --overwrite
  # 생성된 manifest bundle을 CI나 배포 절차가 읽을 경로로 내보낸다.

─── 3일차: DevOps Engineer (배포) ──────────────────────────────────

  (deploy-check에 해당 — MVP: 14-deployment-readiness-checklist.md 수동 이행)
  사용자: checklist 이행 — Secret 리소스 사전 생성, 이미지 build/push,
          server-side dry-run → apply → Pod Running/Ready 확인
  → Pod Ready 도달                                    [Level 2 확정]

  (smoke-test에 해당 — MVP: 15-smoke-test-plan.yaml 수동 실행)
...
─── 실패가 났다면 ──────────────────────────────────────────────────
  ...
  (repair에 해당)
  Pod CrashLoopBackOff → 이벤트/로그 수집 → 16-repair-suggestions.yaml
  → 규칙 기반 항목(env 누락 점검) 또는 LLM patch 제안(Intent/Profile 대상) 검토
  → 사용자가 채택 → 갱신된 Profile/answers로 resume 또는 prepare 재실행 → 재배포
  → 3회 반복 초과 시 escalate (시도 내역은 파일로 보존)
```

커맨드와 단계·산출물의 대응:

| 커맨드 | 대응 Step | 주 산출물 | MVP 상태 |
|---|---|---|---|
| `prepare` | source→analyze→plan/profile→generate→validate 가능한 곳까지 | `analysis/`, `agent/`, `profile/`, `generated/`, `validation/` | 제공 |
| `resume` | 중단 run 재개, source drift 정책 적용 | 갱신된 run artifact | 제공 |
| `status` | run 결과 요약 | final report view | 제공 |
| `explain` | 결정·필드 근거 설명 | evidence refs와 trace | 제공 |
| `export` | 생성 manifest 내보내기 | export directory | 제공 |
| `analyze` | source→phase1→topology | `analysis/` | 단계별 실행용으로 제공 |
| `plan` | intent/question/profile 생성 | `agent/plan.yaml`, `agent/questions.yaml`, `profile/deployment-profile.yaml` | 단계별 실행용으로 제공 |
| `generate` | 템플릿 렌더링 | `generated/manifest-bundle.yaml` | 단계별 실행용으로 제공 |
| `validate` | YAML/internal/kustomize/kubeconform 검증 | `validation/13-validation-report.yaml` | 단계별 실행용으로 제공 |
| `deploy-check` | 13 | report 갱신 (Level 2) | checklist 기반 수동 → 2단계 자동화 |
| `smoke-test` | 14 | report 갱신 (Level 3) | plan 파일 기반 수동 → 2단계 자동화 |
| `repair` | 15 | repair suggestions | 제한적 repair 모듈 존재, 자동 루프는 2단계 확장 |

UI를 얹는 경우에도 이 흐름은 동일하다: 화면은 (a) Reconciliation 결과와 Component 수정 폼, (b) 질문 목록에 답하는 폼(= Profile 작성), (c) Intermediate Model·근거(source/confidence/evidence_refs) 열람, (d) Level 진행 상태 표시의 네 가지를 제공하는 얇은 계층이며, 판정과 산출물은 전부 파일 기반 파이프라인이 만든다 — 비대화형 CI에서도 같은 결과가 나오는 이유다.
