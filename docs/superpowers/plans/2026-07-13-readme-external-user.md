# README External User Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `README.md` so an external user can understand, install, run, and evaluate the Kubernetes manifest pre-analysis workflow from the repository front page.

**Architecture:** This is a documentation-only change. `README.md` becomes the external-user entry point, with setup and sample execution first, optional LLM integration clearly separated, and internal architecture/status moved later. No production code, test code, or package metadata should change.

**Tech Stack:** Markdown, existing Python CLI (`python -m preanalyzer.cli analyze`), `uv` editable install, project-managed `kubeconform`, optional OpenAI-compatible semantic LLM provider.

## Global Constraints

- Main README explanation language is Korean.
- Section headings, commands, paths, and environment variable names must be easy to scan for non-Korean readers.
- Do not describe the tool as a full production deployment automation system.
- Quick Start must run without external LLM setup.
- LLM integration must be documented as optional: explain `--semantic-mode openai_compatible`, `SEMANTIC_LLM_*`, `/models` model-id discovery, and the unauthenticated local endpoint placeholder case.
- Preserve architecture invariants: deterministic first, intermediate models before YAML, evidence before conclusions, secret safety, and bounded semantic agent.
- Use implementation-truth output paths from `src/preanalyzer/pipeline.py`: `11-deployment-profile.yaml` and `12-generated-manifests/`, not the older design names `11-deployment-profile.template.yaml` or `12-rendered-manifests.yaml`.
- Do not touch existing untracked files unrelated to this README task.

---

### Task 1: Rewrite README For External Users

**Files:**
- Modify: `README.md`

**Interfaces:**
- Consumes: `src/preanalyzer/cli.py` command interface: `analyze <repo> [--profile P] [--out DIR] [--ref REF] [--semantic-mode disabled|openai_compatible] [--no-llm]`.
- Consumes: `pyproject.toml` dependency list through editable install.
- Consumes: `src/preanalyzer/pipeline.py` output paths: `00-repository-snapshot.yaml` through `15-smoke-test-plan.yaml`, including `12-generated-manifests/`.
- Produces: A README that external users can follow without reading internal design docs first.

- [ ] **Step 1: Replace the current README structure**

Replace the existing `README.md` with this complete content:
...
- [ ] **Step 2: Verify output names against implementation**

Run:

```bash
rg -n "11-deployment-profile|12-generated-manifests|12-rendered|deployment-profile.template" README.md src/preanalyzer/pipeline.py
```

Expected:

- `README.md` mentions `11-deployment-profile.yaml`.
- `README.md` mentions `12-generated-manifests/`.
- `README.md` does not mention `11-deployment-profile.template.yaml`.
- `README.md` does not mention `12-rendered-manifests.yaml`.
- `src/preanalyzer/pipeline.py` confirms `11-deployment-profile.yaml` and `12-generated-manifests`.

- [ ] **Step 3: Verify LLM is optional but documented**

Run:

```bash
rg -n "LLM은 필수가 아닙니다|--no-llm|--semantic-mode openai_compatible|SEMANTIC_LLM_BASE_URL|curl \"\\$SEMANTIC_LLM_BASE_URL/models\"|SEMANTIC_LLM_API_KEY=\"none\"" README.md
```

Expected: every searched phrase appears in `README.md`.

- [ ] **Step 4: Verify user-facing sections exist in order**

Run:

```bash
rg -n "^## (What is this\\?|When to use it|Quick Start|Run on your repository|Optional LLM integration|What to inspect first|Current status and limitations|Development|Project structure)" README.md
```

Expected: headings appear in this order:
...
- [ ] **Step 5: Run document whitespace check**
...
- The README starts with external-user purpose and use cases.
- Installation uses editable install (`-e .`) instead of listing only partial dependencies.
- Quick Start can run without external LLM setup.
- Optional LLM integration has environment variables, `/models` discovery, placeholder-key guidance, and `--semantic-mode openai_compatible`.
- Limitations state that production deploy automation and Step 13-15 automation are incomplete.
...
```bash
git add README.md
git commit -m "docs: make readme external-user friendly"
```
